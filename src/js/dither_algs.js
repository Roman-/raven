...
function closestColorMethod(imageData, palette, lightCoeff = 0) {
    ...
    function colorDistanceSL(arr1, arr2) {
        let r = Math.abs(arr1[0] - arr2[0]) / 255;
        let g = Math.abs(arr1[1] - arr2[1]) / 255;
        let b = Math.abs(arr1[2] - arr2[2]) / 255;
        let l1 = 0.2126 * arr1[0] + 0.7152 * arr1[1] + 0.0722 * arr1[2];
        let l2 = 0.2126 * arr2[0] + 0.7152 * arr2[1] + 0.0722 * arr2[2];
        let l = Math.abs(l1 - l2) * lightCoeff;
        return r + g + b + l;
    }
...
function orderedDither(imageData, palette, ratio) {
    ...
    let r, g, b, a, i, color, approx, tr, tg, tb;
...
function errorDiffusionDither(imageData, palette, ratioDenom = 3) {
    let d = new Uint8ClampedArray(imageData.data);
    let out = new Uint8ClampedArray(imageData.data);
    ...
    let r, g, b, a, q, i, color, approx, tr, tg, tb, dx, dy, di;
...
function atkinsonDither(imageData, palette, ratioDenom) {
    ...
    let ratio = 1/(ratioDenomScaled*8/3); // default is 1/8
...
/// @returns Euclidean distance between colors
